...
<ParamField body="Charge window start date" param-type="date" required="false" show-location="false">
The overall date the recurring charges are allowed to begin. Campium does not run a recurring charge before this date, even if the cadence would otherwise fall due.
</ParamField>

<ParamField body="Charge window end date" param-type="date" required="false" show-location="false">
The overall date the recurring charges stop. Once this date passes, Campium no longer runs scheduled charges for the membership.
</ParamField>

<Callout kind="info">

The charge window **start** and **end** dates gate the whole recurring schedule — they decide the first and last dates any charge can run. This is different from the **weekly** start date, which only sets the anchor day for counting a weekly cadence.
...
## Set up membership seasons across your team

Membership seasons and recurring membership setup are available to your admin users like any other season tool. There is no longer a super-user restriction on this setup.

Keep these setup rules in mind:

- You can set **Membership season** when creating or editing a season
- Membership seasons appear in season dropdowns for all users, not only super users
- Any user editing a product in a membership season sees the recurring-billing fields and can configure them

<Callout kind="info">

Even though every admin user can reach these settings, it is still a good idea to configure and test one membership season before rolling recurring billing out more broadly.
...
<Columns cols={2}>
  <Card title="Set up seasons" href="/seasons" icon="settings" cta="Review season setup">
    Confirm your base season structure before you add membership billing.
  </Card>
  <Card title="Manage sessions" href="/sessions" icon="edit" cta="Configure session details">
