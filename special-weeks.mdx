...
| A `weeks` profile field exists with week options | Admin → Profile Fields | Attendance reads the camper's selected week options from this field |
...
- The Special Weeks module is enabled in **Admin → Site Settings → Modules**
...
## Week choices on registration

When Special Weeks is enabled and the `weeks` field has options, families pick weeks on the registration products step. They see the heading **Which weeks will you attend?** and a checkbox for each week option.

When a session or week option has start and end dates, those dates can appear under the name on the products page (for example `Jun 22 - Jun 26, 2026`). Dropdown options can show the same range in parentheses after the name. Rows without dates show the name only.

### Exact week count

Week checkboxes start disabled with the help text **Select a session first.** After the family chooses a session product (not only a payment plan), the help text updates live. For a 4-week session with 2 weeks checked, it reads **Select exactly 4 weeks for your session. You've picked 2 of 4.**

- Families must check exactly that many weeks before **Continue** works.
- The hint under Continue counts down, for example **Select 1 more week to continue.** When the count is right, the hint clears.
- If they try to check another week after reaching the required count, Campium explains how to swap: **You've already picked all 4 weeks. Uncheck one to choose a different week.** (For a 1-week session: **You've already picked your week. Uncheck it to choose a different week.**)
- Saving with the wrong count still shows **Select exactly N weeks.**
- When a session's **Number of Weeks** equals every dated week for the season, Campium checks every week for the family automatically. The help text becomes **This session covers all 8 weeks, so every week is selected for you.** (For a one-week season: **This session covers the only week, so it's selected for you.**) If the family unchecks a week, the ordinary count copy returns, and Campium does not re-check weeks for them after that.

Staff see the same weeks block in form **Preview** and in Guided setup under **Preview → Sessions selection**, so you can verify the count gate before families register.

Those checkboxes only appear when the products page loads the Special Weeks block correctly. If families can buy sessions but never see week checkboxes, confirm the module is on, the season has Special Weeks dates, and the form uses the products step with Special Weeks support.

Staff can still set the same week options on the camper profile. Registration checkboxes are the family-facing path during checkout.

### What families see under Current Enrollments

On the parent portal home, under each camper's **Current Enrollments**, families see the season heading and each session name. When Special Weeks is on and the session has two or more weeks with week answers on file, attending weeks list **one week per line** under that session in muted text: the week name, or the week name plus date range such as `Week 1 (Jun 22 - Jun 26, 2026)`. Weeks are ordered by Special Weeks start date. Wait-listed session lines can still show **(Wait List)**. Food lines and single-week sessions do not show a weeks list.

### Edit weeks on a camper profile

On a camper **Sales** tab, staff can change weeks for an eligible session sale without switching the session product.

1. Open the camper profile and go to **Sales**.
2. On a qualifying session line, click the weeks value (dashed underline; title **Edit weeks**), or click **edit weeks** when no weeks are set yet (**None set**).
3. In the modal, choose weeks under **Weeks for {season}**. Helper text: **Choose the weeks this camper will attend. The session product does not change.**
4. Use **History** to pick a season and review **Week**, **Status**, and **Date**. If there are no rows: **No week changes recorded yet.**
5. Click **Save**.

This appears for session sales with two or more weeks when Special Weeks is on. Food and single-week lines do not show the editor. See [Profiles](/profiles#edit-weeks-on-a-session-sale).

### Session weeks vs dated weeks warning

When Special Weeks is on, Campium compares each session's **Number of Weeks** to how many week options have start and end dates for that season. If a session asks for more weeks than families can pick, a red alert appears on:

- the season sessions list
- the **Special Weeks** calendar page
- Guided setup on the **Sessions** step

The summary looks like **N sessions ask for more weeks than families can pick. Only X weeks have dates for this season…**. Listed sessions read **Session name asks for N weeks.**

If no week has dates yet, the alert stays hidden. That case is treated as setup still in progress, not a mismatch.

#### Fix a mismatch

Use either path:

1. **Set special week dates** opens the Special Weeks calendar so you can add start and end dates until the dated count matches each session.
2. **Lower Number of Weeks to N on M sessions** (or **Lower Number of Weeks to N on this session** when only one is listed) lowers the session setting to match the current dated week count. Confirm with **Lower to N weeks**. A success toast reports **Lowered Number of Weeks to N on M sessions.** and the alert closes without a full page reload. You can raise **Number of Weeks** again later on any session.

On the Special Weeks calendar page, the lower-weeks button appears alone. On the sessions list and Guided setup, it appears next to **Set special week dates**.

Families cannot finish registration for a mismatched session until you fix it.
...
<ExpandableGroup>
  ...
  Check **Admin → Profile Fields** and confirm the internal key is exactly `weeks`.
