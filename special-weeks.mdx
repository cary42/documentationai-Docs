...
Week checkboxes start disabled with the help text **Select a session first.** After the family chooses a session product (not only a payment plan), the help text becomes **Select exactly N weeks.**, where N is that session's week count.

- Families must check exactly that many weeks before **Continue** works.
- The hint under Continue reads **Select exactly N weeks to continue.**
- Saving with the wrong count shows the same **Select exactly N weeks.** message.
