...
Each role also offers a few actions:

- **Members** opens a list of the staff users who currently hold that role, so you can see exactly who is affected before you change or remove it.
- **Delete** removes a role. This button stays disabled while any users are still assigned to the role. Reassign those users to a different role first, and then the Delete button becomes available.

<Callout kind="info">

Super admins also see a **Permission Catalog** tab on this screen. It lists every permission the product defines and is used for internal setup, so most staff will not see or need it.
...
Permissions are grouped into categories so you can turn on just the access a job needs. You do not have to grant a whole category; you can enable individual permissions within it. The main categories include:

- **Financials** — view and manage payments, invoices, refunds, and financial reports.
- **Reports** — access to reporting, including which specific reports a role can open.
- **Communication** — send email, SMS, and one-way messaging.
- **Attendance** - take and review attendance in the admin portal and show the **Attendance** tab in the Campium mobile app.
- **Products & Sessions** — manage products, sessions, and enrollment.
- **Bunking** — assign and manage bunk and cabin placements.
- **Medical** — view and update health records.
- **Transportation** — manage transportation and route assignments.
- **Custom fields** — view and edit custom profile fields.
- **Site settings** — change account-wide settings and configuration.
- **Users** — view, edit, create, and merge user records.
- **Roles** — manage roles and permissions themselves.
- **Contracts** — view and manage contracts.
- **Canteen** — manage canteen accounts and spending.
- **Photos** — view and manage photo uploads and galleries.
- **Calendar** — view and manage calendar entries.

Within a category, permissions are often split by action. For example, **Communication** separates email, SMS, and one-way messaging, and **Users** separates viewing, editing, creating, and merging. Grant only the actions a role needs rather than the whole category.
