...
If no registration form in that season has payment plans turned on, the page shows: **Plans will not show on the portal until a form has payment plans enabled. Open the form's Settings and turn on payment plans.**

</Callout>

When the season has no plans yet, the page shows **No payment plans yet**, with: **Payment plans are optional. Add one if parents should pay over time instead of only paying in full.** The button is **Add New Payment Plan**.

## Preview a sample schedule on the list

Above the plans table, **Preview a balance of** lets you enter a sample dollar amount (default `$1,000`). Helper text says: **Each plan below shows what a family owing this much would be charged, and when.**

The **Preview** column (between **Plan Name** and **Type**) shows that sample schedule for every plan, Standard and Even Split alike. Hover the column header for: **What a family owing the balance above would be charged, and when.** Rows can show **per camper** or **balance** badges. Long schedules use **Show N more** / **Show fewer**.

If a plan cannot be previewed yet, the cell explains why (for example missing a final payment date, missing plan details, every charge date in the past, or nothing due on that sample balance). Changing the sample balance refreshes every row.

Use **Copy All To Another Season** to copy every plan in the current season. When a cart-style preview link is available, use **Preview Cart** (it opens in a new tab). That button is separate from the **Preview** column on the list.
...
Available for **Even split** plans only. Choose **Monthly**, **Weekly**, **Bi-weekly**, or **1st & 15th**.
</ParamField>

<ParamField body="Day of month for charges" param-type="integer" show-location="false">
Used with **Monthly** Even Split plans. Choose which day of the month charges should run by default.
</ParamField>

<ParamField body="Let families choose their charge day" param-type="boolean" show-location="false">
Available only for **Monthly** Even Split plans. When checked, checkout shows **Charge day of the month** so the family can pick a day from the **1st** through the **28th**. The plan's **Day of month for charges** stays the default until they change it. After checkout that creates their schedule, the day locks for the family. You can still change it on their profile. Plans with this option show a **Family picks** badge on the payment plans list.
</ParamField>
...
<ParamField body="Require paid by session start" param-type="boolean" show-location="false">
Gates when the plan can be offered so the plan is fully paid before camp begins. When this is on, the plan is only available if its schedule allows the last charge to land on or before the session start date. If the remaining time is too short for the plan to finish in time, families will not see the plan.
</ParamField>

<ParamField body="Show Only For These Sessions" param-type="multi-select" show-location="false">
Restricts the plan to specific sessions. When you select one or more sessions here, the plan is offered only when the cart includes at least one of those selected sessions. Leave it empty to offer the plan for any eligible cart. The label reflects your product terminology, so it may read as **Show Only For These Sessions** or another product name.
</ParamField>
...
#### Let families choose their charge day

On a **Monthly** Even Split plan, you can check **Let families choose their charge day** under **Family chooses the day**.

- At checkout, families see a **Payment plan** card with each plan as a choice, then **Charge day of the month** with shortcut buttons for **1st**, **15th**, and **28th**, plus a dropdown for any day from the **1st** to the **28th**, and helper text: **Your payments will run on this day each month. You will not be able to change it after you check out.**
- The **How much today?** card sits with **Full amount**, pay-later / minimum / **Another amount** options as the form allows, plus **Total today**.
- The schedule preview updates when they change the day.
- After the checkout that creates their scheduled payments, the portal shows locked text such as **Charges on the 15th of each month.** with **To change this, contact the camp office.**
- Weekly, bi-weekly, and 1st & 15th Even Split plans do not offer this option. Standard plans do not either.

This is a per-plan setting. It is not limited to a specific payment processor.

<Callout kind="tip">

Use **Preview a balance of** on the payment plans list to check how Standard and Even Split charges fall between the start and end dates before families use a plan.
...
<Steps>
  ...
  When you save a plan detail from the modal, use **Save and add another** to keep the modal open for the next installment instead of closing and reopening it.
...
- **Require paid by session start** is on and there is no longer enough time for the last charge to land on or before the session start date.
- **Show Only For These Sessions** is set and the cart does not include one of the selected sessions.
...
<Steps>
  ...
  <Step title="Adjust Charge day when needed" icon="calendar">

  For a monthly Even Split plan, use **Charge day** to keep the family's day, pick **Use plan default**, or choose another day (**1st** through **31st**). Changing the day rebuilds their scheduled payments. Helper text explains when the family locked the day at checkout.
...
Charges on the day of the month you set, or on the family's chosen day when **Let families choose their charge day** is on for that plan.
</ParamField>
...
<ParamField body="1st & 15th" param-type="cadence" show-location="false">
Charges on the 1st & 15th of each month when those dates fall within the selected date range.
</ParamField>
...
<ExpandableGroup>
  ...
  Check the plan's start date, end date, and cadence. Weekly and bi-weekly plans use the start date as the repeating anchor, monthly plans use the selected day of month, and 1st & 15th uses those calendar dates within the range.
...
<Columns cols={2}>
  ...
  <Card title="Payment Methods" href="/payment-methods" icon="wallet" cta="Open page" />
