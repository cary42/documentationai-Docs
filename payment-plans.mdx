...
In the plans table, the column header **Preview for** includes a `$` sample balance input (default `$1,000`). Tooltip on the label: **What a family owing this sample balance would be charged, and when**. The same control appears in **Seasons → Guided setup → Payment plans**.

Each row under that column shows the sample schedule for that plan, Standard and Even Split alike. Rows can show **per camper** or **balance** badges. Long schedules use **Show N more** / **Show fewer**.
...
Use **Copy All To Another Season** to copy every plan in the current season. When a cart-style preview link is available, use **Preview Cart** (it opens in a new tab). That button is separate from the **Preview for** column on the list.
...
Use the **Preview for** sample balance on the payment plans list to check how Standard and Even Split charges fall between the start and end dates before families use a plan.
