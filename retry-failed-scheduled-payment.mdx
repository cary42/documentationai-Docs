...
<Steps>
  ...
  Each row shows the scheduled date, amount, error text, and the payment method that was used if one was attached to the original attempt. On Finix and Payabli billing pages, the Error column can also show a short next-step line under the processor message (for example, update the card on file, then retry).

  </Step>
</Steps>

On Finix and Payabli accounts, the parent **Account** page can also show failed scheduled payments in the payment list **Status** column (**Failed** badge, reason, optional next-step text, and a **Manage payment methods and retry** link). USIO billing shows the **Failed Payments** section with the raw error message and does not add the guided next-step line.
...
If the saved card is expired or was declined, add a new method or update the existing one before you retry. Retrying with the same invalid method usually fails again. When Campium has blocked repeats of the same decline, follow the on-screen message to use a different method or contact the camp office.
...
<Steps>
  ...
  Click **Retry**, then confirm when prompted. The confirm can include the payment amount and the method you selected.
  ...
  - **Failure** - the processor declined the charge again. Read the error and any next-step line, update the payment method if needed, and retry again or contact your camp office.
...
<Steps>
  ...
  Failed scheduled charges show a **Failed:** message next to the line item and include a **retry** link. A badge such as **Declined, no automatic retry** means Campium will not retry that charge on its own; you can still use **retry** manually.
...
| Staff retries from a list of failed charges | Admin portal → **Reports** → **System Reports** → **Scheduled Payments** → retry icon |
