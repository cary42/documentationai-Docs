...
- Whether a later successful payment has since been received from the family, so you can tell at a glance which returns are already resolved.
...
- **Payment since?**\
  Shows whether the family has made a later successful payment since this ACH was returned. Use it to spot returns that may already be resolved (a valid replacement payment has come in) versus those that still need follow-up before you archive them.
