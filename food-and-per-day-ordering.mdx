...
The cutoff sets the lead days required before a date. A day is orderable only when its date is later than today plus the lead days, so today itself is never orderable:

- **0** means today is locked and the earliest day families can order is **tomorrow**
- **1** means families must order **2 days ahead**
- **2** means families must order **3 days ahead**

Choose the smallest lead time your staff can reliably fulfill. A shorter window gives families more flexibility, but it also increases same-week changes for your team.
...
- **"No weeks are open for ordering yet."** appears when no defined weeks are currently orderable
...
Two separate settings decide whether a day is orderable. The **order cutoff (lead days)** is a product-level setting that locks any date too close to today. The per-week **Orderable from** and **Orderable until** dates are a separate mechanism that opens or closes an entire week's ordering. A day shows up as orderable only when it clears both.
...
- **Choose realistic cutoff days.** A cutoff of `0` locks today and lets families order from tomorrow onward, while `1` requires ordering 2 days ahead. Match the setting to kitchen or office capacity.
