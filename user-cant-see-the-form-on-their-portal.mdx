...
On the parent portal Forms list, adult cards (parent, guardian, staff, volunteer) with no active forms are hidden, so the section can look empty even when camper cards would still show. Camper or student cards can still appear with the empty message above when they have no forms.
