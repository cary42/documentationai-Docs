...
description: >-
  Assign campers to bunks or cabins, rename the term your camp uses, move
  placements, review requests, and manage capacity warnings from the assignments board.
---
## Place campers into bunks from one board

Bunk Assignments helps you see who still needs a placement, who is already placed, and move people between the unassigned list and named bunks. You will find it in the admin site under **Bunking → Bunk Assignments** (or your renamed plural group, such as **Cabins → Cabin Assignments**).

The board is scoped to one season at a time. Use the season selector in the page header. It opens on your signed-in **Default Season** when that season is available. The page title includes the season name. Only campers with an active enrollment in a session that shows on attendance for that season appear on the board. Buying an add-on alone does not count.

Season is not offered as a **Filter Users** / **Add Another Filter** field here, so the pool cannot disagree with the header season.

This guide uses **bunk** as the default word. If you set a custom term in **Bunk Settings**, the same screens use your singular and plural labels instead.

<Callout kind="info" collapsed="false">
  Staff can use this page only if the **Bunking Module** is on in **Admin → Site Settings → Modules** and their login includes the **Bunking** permission. If you see an access message instead of the board, ask an administrator to confirm both.
</Callout>
...
## Rename bunk, cabin, or group

Camps call these placements different things. Open **Bunking → Bunk Settings** and use the **Terminology** card.

1. Enter a **Singular** word (for example Cabin or Group).
2. Enter a **Plural** word (for example Cabins or Groups). Leave plural blank to add an **s** to the singular.
3. Select **Save Terminology**.

Leave both fields blank to return to the default **Bunk** / **Bunks**. Letters, digits, spaces, and hyphens are allowed. The preview under the fields shows how labels will look.

After you save, the left menu, page titles, buttons, toasts, print modals, label and Excel filenames, and Smart Assignments wording use your term. The top-level menu stays **Bunking** when you use the default. With a custom term, that menu group uses your plural (for example **Cabins**). The summary page follows your singular term (**Bunk Summary**, **Cabin Summary**, and so on).
...
<Callout kind="tip" collapsed="false">
  The user type filter defaults to campers so you are not flooded with staff or parents. That default matches the stored camper user type even when your camp relabels campers on screen. If the results look wrong, check the user type filter first, then select **Apply Filter**.
</Callout>
...
<Steps>
  ...
  <Step title="Choose Allowed Bunks" title-type="p">
    Under **Placement Rules**, use the single **Allowed Bunks** list (or **Allowed Cabins**, and so on, if you renamed the term). Select one or more bunks to limit the draft, or leave it empty to allow all bunks. There is no **Add Rule** button; one list is enough.
  </Step>
...
- optional **Section Field** for a parent unit such as Division or Village

- optional capacities

- whether capacity behavior warns or blocks

- whether a section mismatch warns or blocks
...
## Sections (Division or Village)

Some camps keep bunks inside a larger unit, such as a Division or Village, stored on a profile field. Map that parent field once, then tie each bunk to one of its options.

### Set up a Section Field

1. Open **Bunking → Bunk Settings**.
2. Under field mapping, choose **Section Field**. Helper text explains: **Optional. If your bunks sit inside a larger unit such as a Division or Village, select the profile field that stores it. You can then tie each bunk to one of its options below.** (Your bunk term appears in place of "bunks" when you renamed it.)
3. Choose **When a camper's section does not match the bunk**:
   - **Warn** shows a notice and still allows the assignment.
   - **Block** cancels the assignment until the camper's section matches, or until you switch the setting to **Warn**.
4. Click **Save Field Mapping**.
5. When a section field is set, a colors card appears (the heading uses your section field's name, such as **Division Colors**). Pick a color per option. Status text: **Colors save automatically.** Those colors appear on Bunk Summary.

Only dropdown and multiple-choice profile fields appear in the **Section Field** list. Leave it on **None** if you do not use parent sections.

On the bunks card (your bunk plural label), capacity, warning behavior, section assignment, reorder, and rename changes also save as you edit. Helper text includes **Changes save automatically.** Status under the table: **Bunk settings save automatically.** (Your bunk singular label appears in place of "Bunk".)

### How sections appear on the board

With a section field configured:

- Bunks group under a heading for each section option, then a **No Division** style bucket for bunks with no section (the label uses your section field name).
- The **Show** filter lists bunks under section optgroups.
- Campers whose profile section does not match the bunk's section show a mismatch badge. **Block** stops the drop; **Warn** lets it through and shows a notice.
- Smart Assignments never drafts a camper into a bunk from another section.
- Each section header includes a mail control (**Email [section name]**). Click it to open a compose dialog for people in that section's bunks for the selected season. You need **Manage Email** permission. Optional checkbox: **Include staff and guardians in this group**. Fields: **Subject** and **Message**. Buttons: **Open in full editor**, **Close**, **Send Email**. Confirm with **Send to N [label] recipients?** / **Yes, send it**. Emails go to Guardian 1 and Guardian 2 for campers in the audience. Before you send, the dialog shows how many families or staff and how many addresses are in the group (for example **3 families · 5 addresses in [group].**). Use **Show names** / **Hide names** to review recipients. Addresses missing email show **No email on file** and are skipped; if nobody has an address, send stays blocked. The same email pattern appears on **Bunk Summary** section rows, on a session product's row menu as **Email [Session] Group** (your product singular label), and on the session enrollee roster as **Email This Group**.

## Related bunking features

Once campers are placed, related tools help you print, count, and review placements.

### Bunk labels

Bunk labels let you print labels for your bunks, which is handy for luggage tags, cubbies, or posting bunk lists around camp.

### Bunk reports

Bunk reports give you roster and print-ready views of who is assigned to each bunk, so you can hand a printed list to staff or review placements away from the board.

### Bunk Summary

[Bunk Summary](/group-summary) (or **Cabin Summary** when you rename the term) shows camper counts across your configured groups and weeks, with print and Excel export and click-through rosters.
...
<ExpandableGroup>
  ...
  <Expandable title="I cannot place someone into a bunk" default-open="false">
    Check whether that bunk is using a **Block** capacity rule. If it is, free a spot in the bunk or update the limit in Bunk Settings. If you use a **Section Field**, also confirm the camper's section matches the bunk, or switch the mismatch setting to **Warn**.
  </Expandable>
