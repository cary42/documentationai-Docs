...
On the first step, use **What are you importing?** to choose **Campers and families**, **Staff**, or **Parents and guardians only**. That choice tags every row for the rest of the wizard. Header sniffing no longer decides the mode; it only warns when the file looks like a different type than the one you picked.
...
<Steps>
  <Step title="Open the Import Users tool" icon="folder" title-type="p">
    ...
    - Under **What are you importing?**, choose **Campers and families**, **Staff**, or **Parents and guardians only**. Pick **Staff** only for a staff roster. A camper list without parent columns is still a camper list.

    Success check: You see a page that lets you browse for a file or upload a CSV, with the import type set.
  </Step>
  ...
  <Step title="Map CSV columns to Campium fields" icon="settings" title-type="p">
    - Confirm **Importing as** still matches the type you chose. If a yellow **Check the import type:** banner appears, the file headers look like a different roster than your choice. Go back and change **What are you importing?** if needed.
  ...
  <Step title="Preview the import (dry run)" icon="eye" title-type="p">
    - Before anything is written, use the **Preview** step to see a dry run of what the import will do. The title reflects your type (first 5 families, staff, or parents).

    - Confirm the line that says **Importing as** your chosen type. Review the preview counts (families/campers, staff rows, or parent rows) and check that existing records are being matched rather than duplicated.

    - If the preview looks wrong, go back to **Map Fields** (or change the import type on step 1) and preview again.
  ...
  <Step title="Run the full import" icon="rocket" title-type="p">
    ...
    After you confirm, you can close the window. The import keeps running on the server, and Campium emails the account that started it when it finishes (including the filename). The subject and summary counts follow the import type (campers, staff, or parents). Camper and parent imports can also include a **What was imported** people list in that email.
...
<Callout kind="info" collapsed="false">
  After each import finishes, the account that uploaded the CSV receives a completion email for that file. If you do not see it, check your spam or junk folder and any email filters.
</Callout>
