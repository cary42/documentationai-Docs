...
The same two requirements control the **Attendance** tab in the Campium mobile app. Staff who take attendance through **My Groups** still need a group assignment for that tab. See [Mobile App](/mobile-app#attendance).
...
A day cannot be both **Absent** and have check-in or check-out times. See [Absent and check-in cannot both apply](#absent-and-check-in-cannot-both-apply).
...
## Absent and check-in cannot both apply

A camper cannot be marked **Absent** for a day that already has check-in or check-out activity. That keeps the roster, the profile attendance tab, and attendance reports from showing an absent day with arrival times or a pickup person.

| Situation | What happens |
|---|---|
| You click **Mark Absent** after **Check-in** or **Check-out** is already recorded | Campium blocks the change and shows: *This camper has check-in/check-out activity on this date. Undo the check-in and check-out before marking them absent.* |
| You need to mark them absent after a mistaken check-in | Click **undo** next to the check-in (and check-out, if any), then click **Mark Absent**. |
| Someone was marked absent, then they arrive | On the live roster, **Check-in** stays disabled while the row shows **Absent**. Click **undo** next to **Absent**, then **Check-in**. If a check-in or check-out is attempted while **Absent** is still set (for example from the profile attendance tab), Campium blocks it and shows: *This camper is marked absent for this date. Undo the absent flag before checking them in or out.* On the [Attendance Kiosk](/system-report-attendance-kiosk), a successful **Check-in** scan clears the **Absent** flag for that day automatically. |

<Callout kind="info">

Existing past rows that already show both Absent and punches are left as they are. The rules above stop new contradictory records.
...
Pickup features help staff confirm who is allowed to pick up a camper. Turn on **Pickup Module** in **Admin → Site Settings → Modules**.

### On the parent portal

When **Pickup Module** is enabled, families see **Pickup privileges** for each camper in the parent portal. The page title is **Pickup Information for [name]**.

The lead text explains who may pick up the camper. Parents and guardians are included automatically. Families add anyone else under **Authorized Pickups**.

Under **Parents and Guardians**, the family sees each guardian's **Name** and **Pickup** status: **Allowed**, or **Not allowed for pickup**. The signed-in guardian is marked **(you)**. Help text: **Contact the camp with any questions about who is allowed to pick up.**

They can add authorized pickup people, such as grandparents or caregivers, with **Full name (must match ID)**, **Relationship to child (aunt, friend, sitter)**, and **Phone number**.

If you turn off pickup for a guardian on that camper (see below), that guardian sees: **You are not allowed to pick up [camper]. Pickup contacts for this camper are managed by the camp. Contact the camp to make changes.** The staff-only reason never appears on the portal. Nobody can add a pickup contact whose name matches a blocked guardian; the portal shows: **This person can't be added as a pickup contact. Contact the camp for help.**
...
### Block a guardian from picking up a camper

On a camper or family profile, open **More → Pickup Info** (needs **Pickup Module**).

1. Under **Guardians**, find the camper and guardian row. Helper text: **Turn off pickup for a guardian who may not collect a camper. They won't be offered at check-out on the web, app, or kiosk. You'll be asked for a short reason, which stays with staff. Families see only that the guardian is not allowed for pickup.**
2. Use the **Can pick up** switch. **Allowed** means they appear at check-out. **Not allowed** removes them from web, app, and kiosk check-out lists.
3. When turning pickup off, confirm with **Yes, stop pickup** and enter a **Reason (staff only, not shown to families)**. The prompt title is **Stop [guardian] from picking up [camper]?** Placeholder example: **For example: court order on file, dated [date]**. Empty reasons are rejected with: **Enter a short reason so staff know why pickup was turned off.** Success: **Pickup turned off.** Turning pickup back on shows **Pickup allowed.**
4. Staff without edit permission see **Allowed** / **Not allowed** badges instead of the switch.

The **Reason** column shows the staff reason, or **No reason recorded** for older blocks, plus **Turned off by [name] on [date]** when available.

Under **Pickup changes**, every turn-off and turn-on is listed newest first (**When**, **Camper**, **Guardian**, **Change**, **Reason**, **By**). Change badges: **Pickup turned off** / **Pickup allowed**. Help text: **Every time pickup was turned off or back on for a guardian, newest first.**

Under **Authorized Pickups**, a contact whose name matches a blocked guardian shows the badge **Hidden at check-out**. Check-out refuses that person with: **This guardian is not allowed to pick up this camper.** When **Pickup Popup** is on, a yellow notice can list blocked guardians with their staff reasons for the person checking out.
...
| Option | Purpose |
|---|---|
| **Authorized pickups** | Lists people families authorized to pick up each camper. |
| **Blocked guardians** | Lists guardians marked **Not allowed**, with staff **Reason**, who turned pickup off, when, and **Pickup history**. |
| **Season filter** | Limits results to one season. |
| **Session filter** | Limits results to one session when needed. |
| **Display Columns** | Adds extra profile columns on the authorized view. |

Use this report for front-desk binders, dismissal review, or exports before trips. Full column details: [Pickup Report](/pickup-report).

### Pickup selection at check-out

When **Pickup Popup** is enabled, staff choose the pickup person during check-out. That selection appears on the roster and in related reporting. Guardians marked **Not allowed** do not appear in the list.
...
In **Seasons**, click the season name, then edit the session and review these fields:

| Field | Purpose |
|---|---|
| **Start Date / End Date** | The selected attendance date must fall within the session date range. **Start Date** includes helper text **For attendance.** |
| **Days of week** | The session appears only on the weekdays it is scheduled to run. On **New** and **Edit** session, if both dates are set, **Days of Week** is required. |
...
| **Reports → System Reports → Attendance** | Rosters, kiosk, PDF output, pickup lists, and historical reports. |
...
- If the row shows **Absent**, click **undo** next to **Absent** before using **Check-in**.

</Expandable>

<Expandable title="Mark Absent is blocked">

If the camper already has check-in or check-out activity that day, Campium blocks **Mark Absent** and tells you to undo the check-in and check-out first. Click **undo** next to those times, then click **Mark Absent**.
...
| Session dates and attendance visibility | **Seasons** → click the season name → edit session |