...
- **Visibility** shows who can view or use the template: **Everyone at your organization** (shared with all staff) or **Only you** (private to your login). Super users may also see a system/global scope that publishes a template across all sites.
...
<Step title="Preview and confirm the auto-save">

The editor shows **Your template saves automatically.** As you edit, the status updates to **Saving...** and then **Saved just now.** Preview the message to confirm it looks correct. Use **Save** when you want to force an immediate save.
...
- **Visibility** controls who can access the template. Choose **Everyone at your organization** to share it with all staff, or **Only you** to keep it private to your login. Super users may also have a system/global scope that makes the template available across all sites.
...
- **BCC** blind-copies one or more email addresses whenever this template is used to send a message. Recipients on the To line do not see the BCC addresses.
- **Display Login button** adds a login button when the message needs one, so recipients get a clear link back to their portal.
- **File attachment** lets you attach a supporting file that goes out with the template.
...
- Changes save automatically (**Your template saves automatically.** / **Saving...** / **Saved just now.**). **Save** still forces an immediate save.
...
You can also create or update a template from inside a mass email with **Save as template** on the **Email body** panel. That saves the current subject and body into **Admin → Email Templates** (recipients and send settings are not included). If you inserted one of your organization's templates, you can **Overwrite** that template or **Save as new**. System templates cannot be overwritten from this path.
