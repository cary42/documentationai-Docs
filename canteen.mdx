...
Access to the canteen areas is controlled by three separate permissions, so you can give each staff member only the parts of Canteen they need:

| Permission | What it grants |
| --- | --- |
| **canteen_store** | The main **Canteen** point-of-sale screen and **Add Funds**. |
| **canteen_manage** | **Manage Canteen Products** — adding, editing, and organizing items. |
| **canteen_purchases** | **Purchases**, **Deposits**, and **Account Balance** review screens. |

Assign permissions to match each role. For example, checkout staff typically need **canteen_store**, product owners need **canteen_manage**, and finance staff need **canteen_purchases**. If a staff member is missing a canteen screen, confirm they have the matching permission.
...
- Choose how the purchase is paid:
  ...
  - **Credit card** — May appear if your payment setup supports it. Staff enter the card directly on an embedded card field at the register (processed through Payabli or Finix, depending on your setup), rather than being sent to a separate payment page.
...
- **ACH payments** - Void unsettled ACH deposits to cancel the debit. **refund** appears only after the ACH payment settles. If you open **refund** too early, Campium shows that the ACH payment has not settled yet and offers **Void this payment**.
...
<Callout kind="info" collapsed="false">
  Void windows prevent reversals on transactions that have already settled. For ACH, settlement status (not only the same-day void window) controls when **refund** is available. See [Processing Refunds and Voids](/processing-refunds-and-voids#when-refunds-are-available).
</Callout>

When staff need to load funds in the office — for example, a parent pays at the front desk and you need to credit their camper — use the **Add Funds** modal in **Deposits**.

1. Open **Canteen** in the left menu, then select **Deposits**.
2. Click **Add Funds** in the page header. The Add funds modal opens.
3. Search for the user by name and select them. Their current balance appears below.
4. Choose the payment account — Cash, Zelle, a saved card or bank account on the family's file (options load after you pick the user), or **Adjustment**.
5. Enter the amount. The modal shows the current and new balance as you type.
6. Click **Add funds** to save the deposit, or **Cancel** to close without saving.

<Callout kind="info" collapsed="false">
  **Adjustment** is a manual balance change rather than a real payment. Use it to correct a balance up or down — enter a positive amount to add or a negative amount to subtract. This option is gated behind the financial permission (or super-user access), so it only appears for staff who are allowed to make manual balance changes.
</Callout>
...
- **Withdraw Cash** — Next to any user with a positive balance, a **Withdraw Cash** link appears. Click it to open the cash withdrawal screen and pay out some or all of the remaining funds directly. See [Cash withdrawal](#cash-withdrawal) for the full steps.
- Use the table toolbar **Copy**, **Excel**, **CSV**, **PDF**, or **Print** controls to export the full filtered list, not only the page currently on screen. Apply **Filter** first if you want a subset.
...
- **Allow overspend to clear balances** — Lets a charge exceed the available account balance so staff can zero out or clear a balance at checkout, instead of blocking the sale when funds are short. Leave it off if purchases should never take an account negative.
...
<Steps>
  <Step title="Open the cash withdrawal screen" title-type="p">
    Open **Canteen** in the left menu, then select **Account Balance**. Find the camper whose balance you want to pay out and click **Withdraw Cash** next to their name.
  </Step>
...
<Callout kind="info" collapsed="false">
  If staff do not see cash as a payment option at checkout or in the **Add Funds** modal (Canteen → Deposits → Add Funds), check whether **Accept cash** is turned on in **Canteen settings**.
</Callout>
