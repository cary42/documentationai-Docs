...
description: >-
  Build, save, and share custom reports in Campium, including filters, display
  fields, access settings, exports, labels, email, profile sheets, and batch
  edits.
---
...
<Steps>
  ...
  <Step title="Pick who the fields apply to" icon="users" title-type="p">
    - Use the **Campers** (or your camper plural label), **Staff**, and **Both** buttons at the top of the modal to switch which field list you are editing.

    - **Campers** and **Staff** each keep their own selected columns. **Both** shows the combined set.

    - Click **Save Fields** after you finish choosing columns for the scope you are on.
  </Step>
...
- **Labels** – Open the labels modal for the people in your current report matches.
...
From report results, select **Labels** to open the labels modal.

1. Set **Label Type** to **Address Labels** (the default).
2. Under **Name on Label**, choose whose name prints on the first line:
   - **Guardian** (default) – Uses the household primary contact and prints **one label per household**.
   - **Camper** – Uses the person on each report row (name plus the household address) and prints **one label per person**.
3. Choose **Printer Type** (**Avery** sheet labels or **Dymo** roll labels) and the matching label size.
4. Select **Create**.

**Name on Label** appears only when **Label Type** is **Address Labels**. If you switch to **Custom Labels (Profile Fields)**, you pick profile fields instead of a guardian or camper name.

Use **Camper** when you need a label for every camper in the report (for example, cabin mail or camper name tags with the home address). Use **Guardian** when one envelope per family is enough.
...
<Steps>
  ...
  <Step title="Launch Batch Edit" icon="settings" title-type="p">
    ...
    - **Save** stays off until the field options finish loading. If loading fails, you see **Could not load the options for this field. Choose it again to retry.** Some fields show **This field can't be batch edited.**
  </Step>
