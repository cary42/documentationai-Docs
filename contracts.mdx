...
2. Under **User Status After Signing**, choose the account status to apply (for example Active or Pending), or leave **No Change**.
3. When your staff **Application Status** profile field (or the field you mapped for Staff Hub) has options set up, you also see **Application Status After Signing** (the label matches that field's display name). Choose an option, or leave **No Change**.
...
Helper text on the template reminds you that the signer sees the change on their signed copy, and that it is noted on their profile and on the contract.

### What the signer and staff see after a status change

When a template sets a status (not **No Change**) and the value actually changes:
...
- On **Admin → Contracts**, the contract view includes a **Changes on signing** block with who signed and before-to-after lines such as **Account status: Pending to Active**.
