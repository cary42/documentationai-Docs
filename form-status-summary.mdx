...
- The roster includes both active and inactive participants. Anyone who already has a form record for the season stays visible even if their account later became inactive.
...
<Steps>
  ...
  <Step title="Choose the season (required)" icon="calendar" title-type="p">
    ...
    After you select a season, the table will refresh with one row per participant who has a form record for that season and one column for each live form tied to that season.
  </Step>
...
- **Behavior:**
  ...
  - Participant rows include anyone with a form record for that season, including inactive accounts.
  - The season list can include past or otherwise hidden seasons that do not appear in every other dropdown.
...
Once filters are set, the main table shows one row per participant with a form record for the season and one column per live form for the season.
...
For an Administrator roster of enrolled campers with editable form actions and required-form settings, see [Camper Hub](/camper-hub).
