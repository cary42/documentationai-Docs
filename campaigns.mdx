...
- **Name**: The label shown throughout Campaigns and analytics. Click the name to open results and see full details for that campaign. The campaign's optional **Description** shows as a sub-line beneath the name, so internal notes about the purpose, audience, or channel appear here rather than in a column of their own.
- **Status**: Whether the campaign is **Active** or **Inactive**. Active campaigns track clicks and conversions; inactive campaigns still send families to the correct destination but stop tracking new activity.
- **Clicks**: The total number of times people clicked any tracking link for this campaign.
- **Unique**: An estimate of how many different people clicked the campaign links, so you can focus on people reached instead of raw click volume.
- **Conversions**: The count of registrations that Campium connects back to this campaign based on tracked clicks and conversion rules.
- **Created**: The date the campaign was created, which helps you understand how long it has been running and filter reports by time.
...
The addresses shown here, such as `https://portal.campium.com/t.php?cid=123` and `https://ramaquois.com?cid=123`, are illustrative examples. Your real tracking links use your camp's own portal URL and carry utm parameters. Always copy the exact links from your campaign rather than retyping the examples.
...
When you open **View Analytics**, the date range defaults to the campaign's start date through today (the full life of the campaign so far), not the last 30 days.

Use the date picker to narrow or widen that window. Changing the date range updates the summary numbers, breakdown charts, and recent clicks. **Clicks Over Time** plots every day in the selected range so gaps with no clicks still appear on the chart.

## Generate a campaign link from a mass email

You can create a tracked registration link while you write a mass email, without leaving the email editor.

1. Go to **Communications → Mass Emails**.
2. Open an email (or create one with **New Email**), then stay on the **Settings** tab.
3. Save the email if it is new. The **Campaign tracking** section shows **Save the email first, then generate a campaign link** until it is saved.
4. Click **Generate campaign link**.
5. Choose **Website** or **Parent portal**:
   - **Parent portal** tracks through your parent portal and does not need website tracking code.
   - **Website** requires the tracking code on your website.
6. Click **Insert into email** to place the link in the message body, or copy it from the field.
7. If you have the **Campaigns** permission, open **View campaign results** to review clicks and registrations for that link.

After the email is sent, the same **Settings** tab shows **Clicked** and **Registered** cards next to **Sent** and **Opened** when a campaign link exists for that email. **Clicked** is unique visitors, with total clicks underneath. **Registered** is people who finished registration after clicking, with the 30-day same-device matching rule on the info icon.

Clicks and registrations that use the link count toward the campaign created for that email. See also [Mass Emails](/mass-emails#review-send-and-campaign-stats).
