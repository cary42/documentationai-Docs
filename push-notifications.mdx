...
<Callout kind="info" collapsed="false">
  **Push Notifications** only appears when the mobile app is enabled for your camp, in addition to the communication permission on your account. If the mobile app is not enabled, you will not see the menu item even if you have the permission. Ask an account owner or administrator to enable the mobile app if you expect this option and cannot find it.
</Callout>
...
<Tabs>
  ...
  <Tab title="Logs" icon="bar-chart">
    ...
    - **Resend to a single recipient** — Each row in the log has a per-recipient resend action, so you can send the same notification again to one person (for example, if their device was offline) without notifying everyone.
