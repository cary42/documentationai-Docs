...
<Steps>
  ...
  Go to **Admin → Roles & Permissions** or your role management page.
...
- Category, Priority, and Status change emails come from your camp's outgoing email address in **Admin → Site Settings → Basic** (**Contact Email Address**).
...
Staff can work the same todo items from the Campium mobile app after **Enable Todo List** is turned on in **Admin → Site Settings → Mobile App**.
...
<ExpandableGroup>
  ...
  Confirm **Enable Todo List** is turned on under **Admin → Site Settings → Mobile App**.
...
| Enable for the mobile app | **Admin → Site Settings → Mobile App → Enable Todo List** |
| Grant access to a role | **Admin → Roles & Permissions** → edit role → check **Todo List** |
