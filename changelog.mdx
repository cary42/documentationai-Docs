...
<Update label="2026-07-10" description="Live bus tracking with ETA, tipping reminder emails, medication certification, new payment detail report, security hardening, and critical Payabli charge fix" tags={["feature","improvement","security","bugfix"]}>
  ...
  - Admin login is faster — invoice link generation is deferred for current accounts and empty settings are served from cache.

  - Multi-session discounts are now available to all camps instead of being limited to specific organizations. See [Discounts](/discounts).

  - The [Custom Reports](/custom-reports) page now includes a guided tour for first-time users.

  - Strengthened parent portal security with XSS protection and automatic IP blocking for repeated failed login attempts.
