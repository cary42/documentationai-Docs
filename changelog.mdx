...
<Update label="2026-06-05" description="Finix payout settings, bounced email reporting, portal and form copy improvements, and admin reliability fixes" tags={["feature","improvement","bugfix"]}>

  ## New

  - Added a **Finix payout type** setting in [Site Settings](/settings) so camps can choose the payout behavior that matches their accounting workflow.

  - Added a bounced email report for tracking failed message delivery and follow-up needs from [Mass Emails](/mass-emails).

  ## Updates

  - Streamlined the Site Text editor in [Site Settings](/settings) to make portal copy easier to review and update.

  - Improved the unverified email report so staff can review outstanding email verification issues more clearly.

  - Improved the parent portal header layout for a cleaner experience across screen sizes.

  - Clarified [Forms](/forms) application-fee help text so families better understand when fees apply.

  ## Bug Fixes

  - Fixed Portal Settings card visibility so staff see the right configuration cards in [Site Settings](/settings).

  - Prevented staff from deleting their own [Profiles](/profiles) record by mistake.

  - Fixed sibling [Discounts](/discounts) so they no longer create incorrect $0 credits.

  - Made settings saves more consistent across Portal Settings and related [Site Settings](/settings) pages.
