...
- **Description** (shown on the portal site)
- **Option name** (used in admin and reports; editable inline in the table)
...
- A drag handle column (tooltip **Drag to reorder**) so you can set the display order of options on this page. New options append at the bottom. After you drop a row, Campium confirms with **Order saved.**

The order you set here is the order staff see on the Subproducts page.
...
- **Option name**  
  Short internal label for the subproduct. This appears in admin and reports (column tip **Displayed in reports**). Click the blue value to edit inline (tooltip **Edit option name**). When nothing is set, the cell shows **None set**.

- **Description**  
  Text shown on the portal and on invoices for this option (column tip **Shown on the portal and on invoices**). Click to edit inline (**Edit description**). Empty values show **None set**. Use friendly wording for families. On family season invoices, Campium shows the session display name, then this description when it is set (and not a duplicate of the display name). The option name itself is not printed on the invoice line.
...
- **Price**  
  The amount a matching camper will be charged for this option. Click the blue value to edit inline (tooltip **Edit price**). When nothing is set, the cell shows **None set**.

- **Min. Dep.**  
  The minimum deposit required when a family registers for this option. Click to edit inline (**Edit minimum deposit**). Empty values show **None set**.

- **Capacity**  
  The seat limit for this option. Click to edit inline (**Edit capacity**). Leave blank or set `0` for unlimited; the cell shows **None set**.
...
- **Parent site**  
  Whether this option is shown on the parent-facing registration and pricing (**Yes** or **No**). When set to **No**, the option is hidden from families but still appears here for reporting and staff use.
...
Use quick edits when you only need to adjust a name, description, or number.

- In the subproducts table, click **Option name**, **Description**, **Price**, **Min. Dep.**, or **Capacity** for the row you want to change.
- Type the new value.
- Finish the field (for example, press <kbd>Enter</kbd> or click away) to save. Press <kbd>Escape</kbd> to cancel.
...
- In the modal that opens, review and adjust:
  ...
  - **Parent site** (Yes or No) — When set to **No**, this option is hidden from parent registration and pricing; it still appears here for reporting and staff.
...
The edit modal also includes a per-option **Tiers** control for setting quantity-based or date-based pricing on that subproduct. See [Tiered pricing](/tiered-pricing) for how tiers work.
